  ...
  std::unique_ptr<tf2_ros::Buffer> & tf_buffer, rclcpp::Logger logger_)
{
  ...
  if (!transform_acquired) {
    ...
    try {
      ...
      RCLCPP_WARN(
        logger_, "Failed to lookup transform from base_link to livox_frame at time %f: %s. Trying latest transform...",
      ...
      try {
        ...
        RCLCPP_ERROR(
          logger_, "Failed to lookup latest transform from base_link to livox_frame: %s", ex_fallback.what());
        return;
      }
    }
  }
